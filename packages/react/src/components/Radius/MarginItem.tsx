...
function MarginItem(props: MarginItemType) {
  ...
  const { setOperateState,getOperateState,setSubscribe,executeKeyListener } = useOperate();
  const lockedMargin= position==='left'||position==='right'? 'lockedMarginLeft':'lockedMarginTop';
  ...
  const initMargin=useCallback(()=>{
    ...
    if(selectedNode){
      ...
      marginRef.current.dataset.content=marginPosition+':'+formatUnit(elCss[marginPosition]||0);
    }
  },[]);

  useEffect(()=>{
   const unKeySubscribe=setSubscribe(changeChecked,'lockedMargin'+position);
