...
export function useSelect(
	...
	const { key,
		domTreeKeys:selfDomTreeKeys
	} = specialProps;
	const { selectedInfo } = useSelector<SelectType, STATE_PROPS>(
		['selectedInfo','dragSource'], controlUpdate,
	);
	...
	function controlUpdate(
		...
		if (prevSelectedKey && !selectedKey) {
			...
			return isEqualKey(key, prevSelectedKey)||selfDomTreeKeys.includes(prevSelectedKey);
		}
		if (prevSelectedKey && selectedKey) {
			if (isModal) {
				if (
			...
			if (prevSelectedKey !== selectedKey) {
				return (
					...
					||selfDomTreeKeys.includes(prevSelectedKey)&&!selfDomTreeKeys.includes(selectedKey)
				);
			}else {
