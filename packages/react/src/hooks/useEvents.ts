...
import {
	clearSelectedStatus, getDragSource,
	overTarget,
	selectComponent,
...
export function useEvents(nodeRef:RefObject<HTMLElement>,specialProps:SelectedInfoBaseType,isSelected:boolean,propName?:string){
	const {key,domTreeKeys,parentKey,parentPropName}=specialProps;
	const {pageConfig}=useSelector<HookState,STATE_PROPS>(['pageConfig'],(prevState,nextState)=>controlUpdate(prevState,nextState,key));
