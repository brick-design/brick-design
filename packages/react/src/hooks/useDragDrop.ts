...
function dragDropUpdate(
	prevState: DragDropTypes,
	nextState: DragDropTypes,
	key: string,
) {
	const { selectedKey, domTreeKeys } = nextState.dropTarget || {};
	const { selectedKey: prevSelectedKey, domTreeKeys: prevDomTreeKeys } =
		prevState.dropTarget || {};
	...
	if (prevDragKey === dragKey && dragKey == key && parentKey) {
		...
		if (prevDomTreeKeys && domTreeKeys) {
			return (
				(prevDomTreeKeys.includes(parentKey) &&
	...
	return (
		(prevDragKey && !dragKey) ||
		(!prevDragKey && !!dragKey) ||
		(prevDragKey === key && dragKey !== key) ||
		(prevDragKey !== key && dragKey === key)
	);
}

export function useDragDrop(key: string): DragDropStateType {
	const { dragSource, dropTarget } = useSelector<DragDropTypes, STATE_PROPS>(
