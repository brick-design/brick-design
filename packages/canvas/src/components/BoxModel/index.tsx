...
import { changeElPositionAndSize, getElementInfo } from '../../utils';
...
function BoxModel() {
  ...
  const boxChange = useCallback((positions: any, isFlowLayout: boolean) => {
    ...
    const { top: pageTop, left: pageLeft, width, height } = getElementInfo(
      selectedNode,
      isModal,
    );
    const {
    ...
    if (isFlowLayout) {
      ...
      if (!lockedMarginLeft) {
        ...
        changeElPositionAndSize(leftDistanceRef.current, {
          left:
            marginRight > 0 ? pageLeft + width : pageLeft + width + marginRight,
          top: pageTop + height / 2,
