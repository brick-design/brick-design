...
function controlUpdate(prevState: SelectType, nextState: SelectType, key: string,isModal?:boolean) {
  const { selectedKey: prevSelectedKey, propName: prevPropName,domTreeKeys:prevDomTreeKeys } = prevState.selectedInfo || {};
  ...
  if (prevSelectedKey && selectedKey) {
    if(isModal){
      if(!prevDomTreeKeys.includes(key)&&domTreeKeys.includes(key)||prevDomTreeKeys.includes(key)&&!domTreeKeys.includes(key)){
        return true
      }
    }
    if (prevSelectedKey !== selectedKey) {
