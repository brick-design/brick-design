...
function  NCollapse<T,F,D>(props:NCollapseProps<T,F,D>){
	const {categories,header,renderItem,className,panelContentClass,style,headerClass,collapseClass}=props;
	const [openKeys = [], setOpenKeys] = useState<any[]>([]);

	return (<div style={style} className={`${styles['fold-container']} ${className}`}>
	<Collapse
