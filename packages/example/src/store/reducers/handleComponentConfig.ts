...
import {LayoutSortPayload} from "@/store";
...
export function addComponent(state: StateType) {
     ...
     */
    const dragComponentName=get(vDOMCollection&&vDOMCollection.root||dragKey&&componentConfigs[dragKey],'componentName')
    const dropComponentName=get(componentConfigs[selectedKey!],'componentName')
    const {fatherNodesRule} = get(configs.AllComponentConfigs,dragComponentName );
    const {nodePropsConfig}=get(configs.AllComponentConfigs,dropComponentName)
