...
import {deletePropsConfig} from "@/store";
...
function ObjectComponent(props: ObjectComponentPropsType, ref: any) {
  const { fatherFieldLocation, type, tabIndex, form, childPropsConfig, field, isHideDivider } = props;
  ...
  function deletePropsConf(field: string) {
    confirmModal(() =>deletePropsConfig({
      field,
  ...
  return (
    <Form className={isHideDivider ? '' : styles.divider}>
      ...
      <SelectAddProps
        fatherFieldLocation={formatPropsFieldConfigLocation(type, field, fatherFieldLocation, tabIndex)}
      />
    </Form>
  );
}
