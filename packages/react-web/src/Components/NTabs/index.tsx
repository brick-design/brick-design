...
function NTabs(props:NTabsProps){
	...
	const onMoveStart = useCallback(function (event: React.MouseEvent) {
	...
	const onMoveEnd=useCallback(function(event:MouseEvent){
		event.stopPropagation();
	...
	return<>
		<div style={{display:isDrag?'flex':'none'}} className={styles['placeholder-border']}/>
		<Resizeable  {...rest} ref={moveDivRef}  className={resizeClassName}>
	<Tabs
		tabBarExtraContent={<Icon
