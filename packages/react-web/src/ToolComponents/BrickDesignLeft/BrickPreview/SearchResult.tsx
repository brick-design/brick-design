...
export function renderDragItem(
	...
	if(componentInfos){
		return map(componentInfos,(info,index)=>{
			const {desc,props,img}=info;
			return  <DragAbleItem key={desc||componentName+index} img={img} dragSource={{ componentName, defaultProps:props }} />
		})
	}
	return (
		<DragAbleItem key={componentName}  dragSource={{ componentName }} />
	);
}
...
function SearchResult(props:SearchResultProp,ref:Ref<SearchRefType>){
	...
	return <div style={style} className={styles['result-container']}>
