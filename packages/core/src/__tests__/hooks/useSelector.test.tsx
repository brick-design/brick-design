...
describe('useSelector', () => {
  it('测试没有controlUpdate', () => {
    const { result } = renderHook<LegoProviderProps, any>(() => useSelector<any, STATE_PROPS>(['hoverKey']), {
  ...
  it('测试有controlUpdate', () => {
    const { result } = renderHook<LegoProviderProps, any>(() => useSelector<any, STATE_PROPS>(['hoverKey'], () => false), {
      // eslint-disable-next-line react/display-name
      wrapper: props => <LegoProvider {...props} config={config}/>,
    });
    act(() => {
